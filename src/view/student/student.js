...
import { complain, getUser, getComment, complainRefresh } from '../../store/actions'
...
import {getCourses, getExaminer, studentToExaminerOrLecturer, studentToExaminerRefresh, studentToLecturer, studentToLecturerRefresh} from '../../store/actions/'
...
const resultType = [{ 'value': 'Exam Result', 'label': 'Exam Result' }, { 'value': 'Test Result', 'label': 'Test Result'},]
            ...
            <AddIcon />
                ...
                <div>
                    ...
                    <div className="item item--1">
                        {formElementArray.map(formElement => (
                            <Input elementType={formElement.config.elementType}
                                ...
                                message={messages.map((option, i) => (
                                    <MenuItem key={i} value={option.value}>{option.label}</MenuItem>
                                ))}
                                coursetype={ typeof course !== 'undefined'  && Array.isArray(course) && course.map((option, i) => (
                                    <MenuItem onClick={()=>this.setState({ lecturerId: option.lecturerId })}  key={i} value={option.course.courseId}>{option.course.courseTitle +` (${option.course.courseCode})`}</MenuItem>
                                ))}
                    ...
                    <div className="item item--3">
                            <Button
                            valid={controls.complainType.valid === false ? false :
                            controls.message.valid === false ? false : controls.course.valid === false ? false : null}
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        onComplain: (complainType, message, from, to, studentId, courseId) =>
            dispatch(studentToExaminerOrLecturer(complainType, message, from, to, studentId, courseId)),
...
export default connect(mapStateToProps,mapDispatchToProps)(withStyles(ActionsStyles, { withTheme: true })(Student));
